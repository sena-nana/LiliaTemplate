...
默认 `package.json` 和提交版 `yarn.lock` 固定使用 GitHub 上同一个 LiliaUI commit。基础包始终存在，Layer 包只保留当前预设需要的 `@lilia/ui` 或 `@lilia/nana-ui`。普通 `yarn install` 不依赖本机存在 LiliaUI 源码仓库。
