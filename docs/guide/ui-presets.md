...
```bash
yarn ui:status
yarn ui:preset lilia --dry-run
yarn ui:preset nana
yarn ui:preset nana --check
```
...
预设和依赖来源互相独立：先选择预设，再用 `yarn liliaui:local` 或 `yarn liliaui:remote` 切换来源。提交前应回到远端固定 commit，并运行 `yarn install --immutable`。
...
```bash
yarn ui:migrate --check --json
yarn ui:migrate --preset nana --dry-run
```

确认报告中的安全改动、契约不兼容、信息架构复核和恢复反馈缺口后再执行：

```bash
yarn ui:migrate --preset nana
```
